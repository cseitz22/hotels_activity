...
surprising results?
The most common number of adults in bookings is 2. It is odd that there are bookings with zero adults. I'm also surprised a booking has 55 adults. 

```{r frequency of adults in a booking}
table(hotels$adults)
```
...
All the bookings with more than 4 adults were cancelled. This still doesn't address why there are 294 bookings without an adult. 

```{r cancelled bookings}
hotels %>%
...
```{r decode summary stats}
hotels %>%
  filter(
    adr == 5400
  ) %>%
  select(hotel, arrival_date_month, arrival_date_year, adults, babies, children)
